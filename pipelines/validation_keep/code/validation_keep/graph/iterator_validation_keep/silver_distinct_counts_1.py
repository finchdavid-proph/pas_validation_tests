...
def silver_distinct_counts_1(spark: SparkSession, in0: DataFrame, in1: DataFrame) -> DataFrame:
    ...
    if not target_cols:
        return spark.createDataFrame([], "col_name string, silver_distinct_count long")

    # 1) Normalize types once (dates/timestamps → yyyy-MM-dd, others → string)
    dtype = {f.name: f\
        .dataType for f in in1.schema.fields}
    select_exprs = []
    ...
    normalized = in1.select(*select_exprs)
    ...
    out0 = counts_map.select(F.explode("m").alias("column_name", "silver_distinct_count"))
